...
// Signup Endpoint - Simple save without validation
app.post('/api/signup', async (req, res) => {
  try {
    console.log('Received signup data:', req.body);
...
// Login Endpoint - Just save the login data to MongoDB
app.post('/api/login', async (req, res) => {
  try {
    console.log('Received login data:', req.body);
    ...
    const savedUser = await user.save();
    console.log('Login data saved to MongoDB:', savedUser);
    
    res.json({ success: true, message: 'Login data saved', user: savedUser });
